...
INPUT_PATH = "../modified_data/train_unet/"
PREDICTION_STORAGE_PATH = "../modified_data/unet_processed/"

TEST_RESULT_PATH = "../modified_data/cropped_test/"
TRAIN_RESULT_PATH = "../modified_data/cropped_train/"
ADDITIONAL_RESULT_PATH = "../modified_data/cropped_additional/"
...
def predict_on_train_and_additional_images_unet1(nfolds=5):
...
def predict_on_test_images_unet1(nfolds):
    # Predict test
...
def create_crops_from_predictions_unet1():
    if 1:
        # Test part
        avg_reduction = 0.0
        ...
        for i in range(len(files)):
            orig_path = files[i]
            mask_path = PREDICTION_STORAGE_PATH + 'test/' + os.path.basename(files[i]) + '.npy'
    ...
    for i in range(len(files)):
        orig_path = files[i]
        d1 = os.path.basename(os.path.dirname(files[i])) + '/'
        mask_path = PREDICTION_STORAGE_PATH + 'additional/' + d1 + os.path.basename(files[i])[:-4] + '.jpg.npy'
        if not os.path.isfile(mask_path):
            print('Some error', mask_path)
            exit()
...
def create_csv_file_from_predictions_unet1():
    out = open("../modified_data/rectangles_from_unet_v1.csv", "w")
    out.write("id,type,img_shape0,img_shape1,rect_v1_start0,rect_v1_end0,rect_v1_start1,rect_v1_end1\n")
...
if __name__ == '__main__':
    ...
    # create_crops_from_predictions_unet1()
    create_csv_file_from_predictions_unet1()

# Critical: train Type_3 146, 370
